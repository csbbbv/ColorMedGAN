...
parser.add_argument('--name', required=False, default='oasis_color_v15_onlySigmoid_discri',  help='project name')
...
for epoch in range(args.train_epoch):
    ...
    for (x, _), (y, _)in zip(train_loader_src, train_loader_tgt):
        ...
        S_I_feature_fb = torch.sigmoid(edge_feature_fb) * fake_B
        ...
        S_I_feature_fa = fake_A * torch.sigmoid(edge_feature_fa)
        ...
        S_I_feature = real_A * torch.sigmoid(edge_feature_ra)
        ...
        S_I_feature_fa =  fake_A.detach() * torch.sigmoid(edge_feature_fa)
        ...
        S_I_feature_rb =  real_B * torch.sigmoid(edge_feature_rb)
        # pred_real = BD(real_B)
        ...
        S_I_feature_fb =  fake_B.detach() * torch.sigmoid(edge_feature_fb)
