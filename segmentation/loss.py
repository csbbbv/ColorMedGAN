...
class SoftDiceLoss(_Loss):
    ...
    def forward(self, y_pred, y_true):
        ...
        for i in range(1, self.num_classes):
            class_dice.append(diceCoeffv2(y_pred[:, i:i + 1, :], y_true[:, i:i + 1, :], activation=self.activation))
        mean_dice = sum(class_dice) / len(class_dice)
        return 1 - mean_dice


class SoftDiceLossV2(_Loss):
    def __init__(self, num_classes,  activation=None,weight=[0.5, 1.5, 0.5, 1.5, 1], reduction="sum"):
    ...
    def forward(self, y_pred, y_true):
        class_loss = []
        for i in range(1, self.num_classes):
            dice = diceCoeffv2(y_pred[:, i:i + 1, :], y_true[:, i:i + 1, :], activation=self.activation)
